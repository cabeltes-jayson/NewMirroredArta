...
if (isset($_POST['submitt'])) {
    ...
    if ($count > 0) {
        ...
        $sql_insert = "INSERT INTO `poll`(`controlNumber`, `ctrl_num`, `client`, `date`, `gender`, `age`, `region`, `service`, `c`, `cc`, `ccc`, `sqd`, `sqd_one`, `sqd_two`, `sqd_three`, `sqd_four`, `sqd_five`, `sqd_six`, `sqd_seven`, `sqd_eight`, `suggestion`, `email`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        
        $stmt_insert = $conn->prepare($sql_insert);
        $stmt_insert->bind_param("ssssssssssssssssssssss", $controlNum, $ctrl_num, $client, $currentDate, $gender, $age, $region, $service, $c, $cc, $ccc, $sqd, $sqd_one, $sqd_two, $sqd_three, $sqd_four, $sqd_five, $sqd_six, $sqd_seven, $sqd_eight, $suggestion, $email);
        ...
        echo '<script>
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.replace("addonform.php"); 
                }
            });
        </script>';
        } else {
